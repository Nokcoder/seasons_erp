...
2. `sale_pid` is a system-generated human-friendly reference (e.g. `SALE-00123`).
3. `idempotency_key` is provided by the client. If a sale with the same key already exists, the existing sale is returned without creating a duplicate. This prevents double-posting on network retries.
