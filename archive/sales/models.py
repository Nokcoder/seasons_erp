...
class SalesHeader(Base):
    ...
    discount_amount = Column(Numeric(12, 2), default=0.00)  # Total item-level discounts
    tax_amount = Column(Numeric(12, 2), default=0.00)

    basket_discount_amount = Column(Numeric(12, 2), default=0.00)  # Untangled marketing promos
    service_charge = Column(Numeric(12, 2), default=0.00)
    delivery_charge = Column(Numeric(12, 2), default=0.00)

    total_amount = Column(Numeric(12, 2), nullable=False)

    # --- NEW: MATH ADJUSTMENTS & EVENT SOURCING ---
    manual_adjustment_amount = Column(Numeric(12, 2), default=0.00)
    adjustment_reason = Column(String(255), nullable=True)
    linked_receipt_id = Column(Integer, ForeignKey("sales.sales_headers.sales_id"), nullable=True)
    transaction_type = Column(String(50), default="SALE") # 'SALE', 'REFUND', 'EXCHANGE'

    # Customer Tracking
    customer_name = Column(String, nullable=True)
    customer_id = Column(Integer, ForeignKey("sales.customers.customer_id"), nullable=True)
