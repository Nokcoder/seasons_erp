    ...
    <div className="flex h-full overflow-hidden">
      ...
      <aside className="w-72 shrink-0 border-r t-border t-bg-surface flex flex-col overflow-hidden">
        <div className="p-3 border-b t-border">
          ...
          <input className={iCls} placeholder="Brand, variant name, PID, SKU…"
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b t-border">
                {['Brand','Variant','PID','Stock at Source','Bundle Count','Qty',''].map(h => (
            ...
            <tbody>
              {lines.length === 0 && (
                <tr><td colSpan={7} className="px-2 py-8 text-center t-text-4">Search and click items on the left to add them.</td></tr>
              )}
              {lines.map(line => {
