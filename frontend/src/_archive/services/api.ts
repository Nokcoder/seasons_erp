...
export interface Product {
  ...
  is_bundle: boolean;
  is_active: boolean;
  units_per_carton?: number | null; // Fixed from units_per_bundle to match backend
...
//   categories: Category[]; 
//   variants: ProductVariant[];          // 🌟 THE GREAT DECOUPLING LINK
//   bundle_components?: BundleComponent[];
// }
...
export interface ProductCreate {
  ...
  // The Decoupled Arrays
  variants?: VariantCreate[];
  bundle_components?: BundleComponentCreate[];
}
...
export const createVariant = async (data: any): Promise<ProductVariant> => {
...
export const updateVariant = async (variantId: number, data: any): Promise<ProductVariant> => {
...
export interface StockTransferItem {
  item_id: number;
  variant_id: number; // 🌟 CHANGED from product_id
  bundling: string | null;
  requested_qty: number;
  released_qty: number | null;
  received_qty: number | null;
  variant?: ProductVariant; // 🌟 CHANGED from product?: Product
}
...
export const receiveTransfer = async (
  ...
  unexpected_items: { variant_id: number, received_qty: number, bundling?: string }[] // 🌟 CHANGED to variant_id
) => {
...
// 🌟 URL CHANGED: Ledger now checks variantId
export const fetchItemLedger = async (variantId: number, locationId: number) => {
  const response = await fetch(`${API_BASE_URL}/products/variants/${variantId}/ledger/${locationId}`);
...
export interface GRNItem {
  ...
  discount: number;
  net_cost?: number;
  variant?: ProductVariant; // 🌟 CHANGED
}
...
export const fetchSalesDashboard = async (params: { 
  start_date?: string, 
  end_date?: string, 
  search?: string,
  shift?: string,            // <-- ADDED
  register_id?: string | number  // <-- ADDED (Allows string from select, or number)
}) => {
  ...
  // SURGICAL FIX: Append the new dynamic filters if they exist
  if (params.shift) query.append('shift', String(params.shift));
  if (params.register_id) query.append('register_id', String(params.register_id));
...
export const fetchSaleByDocumentId = async (documentId: string) => {
  const response = await fetch(`${API_BASE_URL}/sales/document/${documentId}`);
  if (!response.ok) {
...
export const fetchSettingsData = async (endpoint: string) => {
  const response = await fetch(`${API_BASE_URL}/settings/${endpoint}`);
  if (!response.ok) throw new Error(`Failed to fetch ${endpoint}`);
  return response.json();
};

export const saveSettingsData = async (endpoint: string, data: any) => {
  const response = await fetch(`${API_BASE_URL}/settings/${endpoint}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(data),
...
// (Make sure API_BASE_URL is defined/imported at the top of this file)



export const fetchPayments = async () => {
  try {
    const response = await fetch(`${API_BASE_URL}/settings/payments`);
...
export const fetchRegisters = async () => {
  try {
    const response = await fetch(`${API_BASE_URL}/settings/registers`);
